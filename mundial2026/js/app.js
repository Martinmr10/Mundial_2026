...
const DB = {
  ...
  upsertPick:    (player_name, match_id, outcome, goals1, goals2) =>
    sbFetch("picks", { method:"POST", body: JSON.stringify({player_name, match_id, outcome, goals1, goals2}), headers:{"Prefer":"resolution=merge-duplicates,return=minimal"} }),
  ...
  upsertResult:  (match_id, score1, score2) =>
    sbFetch("results", { method:"POST", body: JSON.stringify({match_id, score1, score2, updated_at: new Date().toISOString()}), headers:{"Prefer":"resolution=merge-duplicates,return=minimal"} }),
  ...
  getLateAccess: async () => {
    ...
    catch(e) { console.warn("late_access missing:", e.message); return []; }
  },
  ...
  getSettings: async () => {
    ...
    return { ptsWinner: parseInt(s.pts_winner??1), ptsPartial: parseInt(s.pts_partial??1), ptsExact: parseInt(s.pts_exact??4), wcApiKey: s.wc_api_key||"" };
  },
...
function resolveGoals(pick) {
  ...
  return { g1: raw1!==null?raw1:0, g2: raw2!==null?raw2:0, hasPrediction:true };
}
...
function calcPoints(pick, result) {
  ...
  if (hasPrediction) { if(g1===s1) pts+=settings.ptsPartial; if(g2===s2) pts+=settings.ptsPartial; }
  return pts;
}
...
async function populateLoginDropdown() {
  ...
  try {
    ...
    players.sort((a,b)=>a.name.localeCompare(b.name)).forEach(p=>{
      const o=document.createElement("option"); o.value=p.name; o.textContent=p.name; select.appendChild(o);
    });
...
async function handleLogin() {
  ...
  try {
    ...
    cachedResults={};(resultsRows||[]).forEach(r=>{cachedResults[r.match_id]={score1:r.score1,score2:r.score2};});
    cachedEnabled={};(enabledRows||[]).forEach(r=>{cachedEnabled[r.match_id]=true;});
...
function renderPhaseSelector(cid,activePhase,isAdmin) {
  document.getElementById(cid).innerHTML=PHASES.map(p=>
    `<button class="phase-btn ${p.id===activePhase?'active':''}" onclick="${isAdmin?`switchAdminPhase('${p.id}')`:`switchPhase('${p.id}')`}">${p.label}</button>`
  ).join("");
}
...
function getResultLabel(pick, result) {
  ...
  const actual=getOutcome(s1,s2);
  const wonCorrect=pick.outcome===actual;
  const g1ok=hasPrediction&&g1===s1,g2ok=hasPrediction&&g2===s2;
...
function setPick(matchId, outcome) {
  ...
  const g1=playerPicks[matchId].goals1,g2=playerPicks[matchId].goals2;
  const hasBoth=g1!==null&&g1!==undefined&&g2!==null&&g2!==undefined;
  if (hasBoth) {
    const derived=g1>g2?"1":g1<g2?"2":"x";
    if (derived!==outcome) {
      playerPicks[matchId].goals1=null;playerPicks[matchId].goals2=null;
      const i1=document.getElementById("g1-"+matchId),i2=document.getElementById("g2-"+matchId);
  ...
  container.querySelectorAll(".pick-btn").forEach(btn=>{
    ...
    if (!oc.includes("'"+matchId+"'")) return;
    btn.classList.remove("selected","correct","wrong","auto-derived");
    if (oc.includes("'"+outcome+"'")) btn.classList.add("selected");
  });
}
...
function setGoals(matchId) {
  ...
  if (hasPrediction) {
    const derived=rg1>rg2?"1":rg1<rg2?"2":"x";
    playerPicks[matchId].outcome=derived;
    const container=document.getElementById("matches-container");
    ["1","x","2"].forEach(v=>{
      const btn=container.querySelector(`[onclick="setPick('${matchId}','${v}')"]`);
      if(btn){btn.classList.remove("selected","correct","wrong","auto-derived");if(v===derived)btn.classList.add("selected","auto-derived");}
    });
  } else {
    const container=document.getElementById("matches-container");
...
async function savePlayerPicks() {
  ...
  allMatchIds.forEach(mid=>{
    ...
    if (hasPrediction&&!p.outcome) playerPicks[mid].outcome=g1>g2?"1":g1<g2?"2":"x";
    if (hasPrediction){playerPicks[mid].goals1=g1;playerPicks[mid].goals2=g2;}
  });
  const saveable=allMatchIds.filter(mid=>{
    const m=ALL_MATCHES.find(x=>x.id===mid),p=playerPicks[mid];
...
function renderAdminMatches(phase) {
  const matches=ALL_MATCHES.filter(m=>m.phase===phase).slice().sort((a,b)=>new Date(a.kickoff||0)-new Date(b.kickoff||0));
    ...
    <div class="match-card admin-match ${enabled?'match-enabled':'match-disabled'}" id="card-${m.id}">
      <div class="match-card-top">
        ...
        <div style="display:flex;gap:6px;align-items:center;">
          ${hasResult?`<button class="btn-reset-result" onclick="resetResult('${m.id}')">🔄 Resetear</button>`:""}
...
async function resetResult(matchId) {
  ...
  if(!confirm(`¿Resetear el resultado de ${name}?`))return;
  ...
  try {
    ...
    if(card&&m)card.outerHTML=renderAdminMatchCard(m);else renderAdminMatches(adminPhase);
    alert("✅ Resultado reseteado.");
  } catch(e){alert("❌ Error: "+e.message);}
  finally{hideLoading();}
}
...
async function syncResultsFromAPI() {
  if(!settings.wcApiKey){alert("Primero guarda tu API Key en CONFIG.");return;}
    ...
    container.innerHTML=players.map((p,i)=>{
      ...
      return `<div class="player-card">
        ...
        <div class="player-stats">
          ...
          <button class="btn-late-access ${hasLateAccess?'granted':''}" onclick="toggleLateAccess('${p.name}',${hasLateAccess})">
...
async function viewPlayerPicks(playerName) {
  ...
  if(!modal){alert("Actualiza el index.html — falta el modal de picks.");return;}
  ...
  try {
    ...
    const picksMap={};(picksRows||[]).forEach(r=>{picksMap[r.match_id]=r;});
    const resultsMap={};(resultsRows||[]).forEach(r=>{resultsMap[r.match_id]=r;});
    const relevant=ALL_MATCHES.filter(m=>picksMap[m.id]||resultsMap[m.id]);
    if(!relevant.length){document.getElementById("modal-picks-content").innerHTML='<div class="empty-state">Este jugador no tiene picks aún.</div>';return;}
  ...
  } catch(e){document.getElementById("modal-picks-content").innerHTML=`<div class="empty-state">Error: ${e.message}</div>`;}
}

function closePicksModal(){document.getElementById("modal-picks").style.display="none";}

// ── Permiso especial para pronosticar tarde ───────────────
async function toggleLateAccess(playerName, currentlyGranted) {
  try {
    if(currentlyGranted){await DB.revokeLateAccess(playerName);lateAccessPlayers.delete(playerName);}
    else{await DB.grantLateAccess(playerName);lateAccessPlayers.add(playerName);}
    await renderAdminPlayers();
  } catch(e){
    if(e.message.includes("42P01")||e.message.includes("does not exist")){
      alert("⚠️ Ejecuta este SQL en Supabase:\n\nCREATE TABLE late_access (\n  player_name text primary key\n);\nALTER TABLE late_access DISABLE ROW LEVEL SECURITY;");
    } else {alert("Error: "+e.message);}
  }
}
